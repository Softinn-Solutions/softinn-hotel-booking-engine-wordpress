...
if ( !class_exists( 'SoftinnBE' ) ) {
	...
	class SoftinnBE
	{
		...
		function __construct() {
			...
			include_once('inc/softinn-widget.php'); //include the widget file
		}
		...
		function register() {
			...
			add_action( 'widgets_init', function(){register_widget( 'Softinn_Widget' );});
		...
		function softinn_enqueue_front(){
			...
			// JS
			wp_enqueue_script( 'softinn-jq-js', plugins_url( '/assets/jquery-3.3.1.min.js', __FILE__ ) );
			wp_enqueue_script( 'softinn-jq-ui-js', plugins_url( '/assets/jquery-ui.min.js', __FILE__ ) );
			...
			wp_enqueue_script( 'softinn-iframe-resize-js', plugins_url( '/assets/iframeResizer.min.js', __FILE__ ) );
